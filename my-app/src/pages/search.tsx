  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      try {
        ...
        const apiUrl = `https://tracuugiadat.1022.vn/api/land-prices?street=${encodeURIComponent(street)}&dvhc=${encodeURIComponent(dvhc)}&segment=${encodeURIComponent(segment)}&limit=9999`;
        
        const response = await fetch(apiUrl);
        const json = await response.json();

        // Lấy danh sách kết quả đã được Backend xử lý
