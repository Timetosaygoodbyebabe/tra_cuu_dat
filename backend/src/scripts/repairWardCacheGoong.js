...
// Tìm các đường chưa xác định phường
const missingStreets = [];
for (const [street, ward] of Object.entries(cache)) {
    if (ward.includes("Chưa xác định")) {
        missingStreets.push(street);
    }
}

console.log(`[GOONG API] Tìm thấy ${missingStreets.length} đường cần xác định lại bằng Goong API.`);
...
async function run() {
    for (let i = 0; i < missingStreets.length; i++) {
        ...
        try {
            console.log(`[${i + 1}/${missingStreets.length}] Goong tra cứu: ${street}...`);
            ...
            if (res.data && res.data.results && res.data.results.length > 0) {
                // Thử tìm trong các kết quả xem có map được với 12 phường không
            ...
            try {
                ...
                for (let j = 0; j < feData.length; j++) {
                    if (feData[j].td === street || feData[j].ten_duong === street) {
                        if (feData[j].pcx !== undefined) feData[j].pcx = finalWard;
                        if (feData[j].phuong_chinh_xac !== undefined) feData[j].phuong_chinh_xac = finalWard;
                        updated = true;
                    }
                }
                if (updated) {
        ...
        // Goong API có rate limit, chờ một chút giữa các request
        await delay(500);
    }
    console.log("[GOONG API] Hoàn tất!");
}
